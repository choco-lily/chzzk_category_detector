...
- **중복 알림 방지**: 동일한 방송 세션에 대해 중복으로 알림을 보내지 않습니다. (로컬 실행 시 메모리 기반)
- **자동 목록 관리**: 방송이 종료되거나 시청자 수가 기준치 아래로 떨어지면 상태를 추적합니다.
- **풍부한 알림**: 방송 제목, 현재 시청자 수, 채널 이미지 및 방송 썸네일을 포함한 임베드 메시지를 전송합니다.
