...
- **실시간 모니터링**: 치지직 API를 통해 특정 카테고리의 라이브 방송 목록을 주기적으로 확인합니다.
- **스마트 알림 시스템**: 
  - 시청자 수가 기준치(예: 1,000명)를 초과할 때만 알림을 보냅니다.
  - **중복 방지**: 한 번 알림을 보낸 방송은 동일한 방송 세션이 유지되는 동안(시청자 수가 등락하더라도) 재알림을 보내지 않습니다.
  - **세션 관리**: 방송이 완전히 종료된 후 다시 시작될 경우에만 새로운 알림을 보냅니다.
- **풍부한 알림 메시지**: 
...
4. **워크플로우 확인**:
   - `.github/workflows/monitor.yml` 설정에 따라 기본 10분마다 자동으로 실행됩니다.
